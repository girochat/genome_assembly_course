...
dat <- fread("analysis/genespace/orthofinder/Results_Nov18/Comparative_Genomics_Statistics/Statistics_PerSpecies.tsv")
ids=names(dat)
...
p <- ggplot(o_ratio, aes(x =  V1, y = perc, fill = species)) +
  ...
  labs(y = "Count")
ggsave("analysis/genespace/plots/orthogroup_number_plot.pdf")
...
p <- ggplot(o_percent, aes(x =  V1, y = perc, fill = species)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(y = "Count")
ggsave("analysis/genespace/plots/orthogroup_percentage_plot.pdf")
...
ogroups <- fread("analysis/genespace/orthofinder/Results_Nov18/Orthogroups/Orthogroups.GeneCount.tsv")
...
genomes <- c("An_1", "C24", "Cvi_0", "Ler_3", "Sha", "TAIR10")  #names(ogroups_presence_absence)[grepl("proteins",names(ogroups_presence_absence))]
...
pdf("analysis/genespace/one-to-one_orthogroups_plot.complexupset.pdf", height = 5, width = 10, useDingbats = FALSE)
